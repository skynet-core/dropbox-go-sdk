...
class GoTypesBackend(CodeBackend):
    ...
    def _generate_base_type(self, base):
        ...
        self.emit(
            '// Is{0} is the interface type for {0} and its subtypes'.format(t))
        ...
        self.emit(
            "// Is{0}FromJSON converts JSON to a concrete Is{0} instance".format(t))
    ...
    def _generate_struct(self, struct):
        with self.block('type %s struct' % struct.name):
            if struct.parent_type:
                self.emit(fmt_type(struct.parent_type,
        ...
        if needs_base_type(struct):
            self.emit(
                '// UnmarshalJSON deserializes into a %s instance' % struct.name)
            with self.block('func (u *%s) UnmarshalJSON(b []byte) error' % struct.name):
                ...
                for field in struct.all_fields:
                    ...
                    tn = fmt_type(dt, namespace=struct.namespace,
                                  use_interface=True)
                    if _needs_base_type(dt):
                        if is_list_type(dt):
                            self.emit(
                                "u.{0} = make({1}, len(w.{0}))".format(fn, tn))
                            # Grab the underlying type to get the correct Is...FromJSON method
                            tn = fmt_type(
                                dt.data_type, namespace=struct.namespace, use_interface=True)
                            with self.block("for i, e := range w.{0}".format(fn)):
                                self.emit(
                                    "v, err := {1}FromJSON(e)".format(fn, tn))
                                with self.block('if err != nil'):
                                    self.emit('return err')
                                self.emit("u.{0}[i] = v".format(fn))
                        else:
                            self.emit(
                                "{0}, err := {1}FromJSON(w.{0})".format(fn, tn))
                            with self.block('if err != nil'):
                                self.emit('return err')
            ...
            for field in struct.all_optional_fields:
                if field.has_default:
                    if is_primitive_type(field.data_type):
                        ...
                        self.emit('s.{0} = {1}'.format(
                            fmt_var(field.name), default))
    ...
    def _generate_field(self, field, union_field=False, namespace=None, raw=False):
        ...
        field_name = fmt_var(field.name)
        type_name = fmt_type(field.data_type, namespace,
                             use_interface=True, raw=raw)
